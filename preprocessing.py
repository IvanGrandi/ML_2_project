...
def process_numeric_column(series, min_val=None, max_val=None):
    """
    Converts a column to numeric, coerces errors, and tracks validity within boundaries.
    """
    # 1. Coerce to numeric (unparseable values become NaN)
    numeric_series = pd.to_numeric(series, errors='coerce')
    
    # 2. A row is valid if it's a number AND within the specified range
    ...
    return numeric_series, valid_mask

def process_date_column(series):
    """
    Converts a column to datetime and tracks validity.
    """
    # 1. Coerce to datetime (unparseable values become NaT)
    date_series = pd.to_datetime(series, errors='coerce')
    
    # 2. A row is valid if it's a real timestamp
    valid_mask = date_series.notna()
    
    return date_series, valid_mask
